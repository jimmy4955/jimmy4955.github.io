'use strict';
import { failSafe } from '../security/domain-lock.js';
import { verifyRuntimeToken } from '../security/runtime-token.js';
...
window.__FWG_BLOCK_GAME__ = blockRuntimeGame;
...
export async function bootGame() {
  try {
    const passed = window.__FWG_SECURITY_PASSED__ === true;
    const tokenOk = await verifyRuntimeToken(window.__FWG_SECURITY_TOKEN__);
    if (!passed || !tokenOk) {
